...
include 'db.inc.php';
$db=mysql_connect(MYSQL_HOST,MYSQL_PASSWORD)or die('Unable to connect.Check your connection parameters.');
mysql_select_db(MYSQL_DB,$db)or die(mysql_error($db));
if(isset($_POST['submit'])&& $_POST['submit']== 'Yes'){
    $query='DELETE i FROM
              site_user u JOIN site_user_info i ON u.user_id=i.user_id WHERE u.username='.
        mysql_real_escape_string($_SESSION['username'],$db).'"';
    mysql_query($query,$db)or die(mysql_error($db));
    $query='DELETE FROM site_user WHERE username="'.
        mysql_real_escape_string($SESSION['username'],$db). '"';
}
mysql_query($query,$db)or die(mysql_error($db));
...
<a><a href="main.php">Click here</a> to return to the homepage.</a></p>
</body>
</html>
